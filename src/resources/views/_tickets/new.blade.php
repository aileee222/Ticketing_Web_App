...
<body>
    <header>
        <h1>
            <span class="playwrite-regular">New</span> Ticket
        </h1>
    </header>
    <div class="main">
        <section class="left">
        ...
        <section class="right">
            <div>New Ticket</div><br />
            <form id="submitTktform" class="form_container" action="{{ route('_tickets.store') }}" method="POST">
                @csrf
                <div class="projet_deadline_container">
                    <div class="project_name_container">
                        <span class="project_name">From Project</span>
                        <input class="project_name_entry" list="available_project" placeholder="choose a project name">
                        <datalist id="available_project">
                            @foreach ($projectsN as $projectN)
                                <option value="{{ $projectN->name }}">
                            @endforeach
                        </datalist> 
                        <div id="name_error" class="error hide">Please add a ticket owner(s).</div>
                ...
                <div class="ticket_name_container">
                    ...
                    <div id="tktname_error" class="error hide">Please add a ticket name.</div>
                </div>
