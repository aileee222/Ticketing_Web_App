...
<body>
    <header>
        <h1>
            <span class="playwrite-regular">My</span> Projects
        </h1>
    </header>
    <div class="main">
      <section class="left">
        ...
        <section class="right">
            <div class="project_title_container">
                <div>Current Projects</div><br />
                <input class="title_container_input" type="text" placeholder="Enter project name">
            </div>
            <div class="projects_container">
              @foreach ($projects as $project)
                  <a href="{{ route('_projects.show', $project->id) }}" class="project_card">
                        <h3>{{ $project->name }} #P{{ $project->id }}</h3>
                        <p>{{ $project->description }}</p>
                        <p>Owner: {{ $project->owner }} | Duration: {{ $project->tlimit }}</p>
                        <p>Status: 
            ...
            @if($userStatus !== 'Developper')
                <button class="confirm_button" onclick="window.location.href = '{{ route('_projects.new') }}';">+</button>    
            @endif
        </section>
    </div>    
